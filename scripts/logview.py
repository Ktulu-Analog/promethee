...
_RE_MERMAID   = re.compile(r"\[Mermaid\]|mermaid_renderer")

# Sous-patterns Mermaid pour l'affichage enrichi
_RE_MERMAID_REJECT  = re.compile(r"Bloc rejeté")
_RE_MERMAID_ACCEPT  = re.compile(r"Bloc #\d+ accepté")
_RE_MERMAID_FIX     = re.compile(r"Correction\s*(?:universelle|structurée|Sankey|IDs réservés|:)")
_RE_MERMAID_SANKEY  = re.compile(r"Sankey")
...
def _module_color(module: str) -> str:
    ...
    elif "mermaid" in m:                   color = C.BR_GREEN
...
def _format_msg(msg: str, level: str) -> str:
    ...
    # Mermaid
    if _RE_MERMAID.search(msg):
        if _RE_MERMAID_REJECT.search(msg):
            # Bloc rejeté → mise en évidence comme un warning même si niveau DEBUG
            return _c("🚫  " + msg, C.BR_YELLOW, C.BOLD)
        if _RE_MERMAID_ACCEPT.search(msg):
            return _c("📊  " + msg, C.BR_GREEN)
        if _RE_MERMAID_FIX.search(msg):
            return _c("🔧  " + msg, C.CYAN)
        if _RE_MERMAID_SANKEY.search(msg):
            return _c("〰  " + msg, C.BR_CYAN)
