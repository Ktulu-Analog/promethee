...
description: >
  ...
  affichés dans la réponse. Graphiques via Matplotlib (export automatique PNG),
...
| Besoin | Mécanisme |
|---|---|
| Graphique de données (courbes, barres, camemberts, nuages de points…) | **Matplotlib** → affiché automatiquement |
...
## 2. Graphiques Matplotlib — affichage automatique
...
Imprimer la data-URI produit une longue chaîne base64 illisible dans le chat, et
l'image que le LLM aurait hallusinée serait corrompue. L'affichage automatique est
fiable, propre et ne consomme pas de tokens de contexte.

### Principe de fonctionnement

`python_exec` intercepte `plt.show()` et `plt.savefig()` : le graphique est sauvegardé
automatiquement dans un fichier temporaire, encodé en base64 par l'application, et affiché
dans la bulle de réponse sous forme de miniature cliquable. Aucun code d'encodage à écrire.
...
Utiliser `print()` uniquement pour des résultats **textuels** (valeurs numériques, résumés, etc.).
...
- `dpi=150` est un bon compromis qualité/taille dans `plt.savefig()` si utilisé.
- `figsize=(8, 5)` est la taille par défaut recommandée pour une bulle de 920px de large.
- **Ne jamais** imprimer de data-URI base64 — utiliser `plt.show()` ou laisser l'auto-save.
...
| Image non affichée | Vérifier que `plt.show()` est bien appelé (NE PAS encoder en base64) |
...
| Base64 illisible dans le chat | Ne jamais `print(f"![...](data:image/png;base64,{b64})")` — utiliser `plt.show()` |
